...
def fool_play():
    '''
    global info,money,dicenumber,firstinto
    level=info[1]
    os.system("cls")
    if firstinto:
        print("Sevensoft Games Present")
        time.sleep(3)
        os.system("cls")
        print("天天爱博彩")
    ...
    cmd=input("每次游戏需要花费$0，继续吗？（按p继续，按e退出）")
    if cmd=='p':
        pass
    else:
        return -1
    os.system("cls")
    show_game_info(0,level)
    print("你已经拿到了后台的骰子控制权，可以随意操控骰子的点数。")
    first=input("押大，押中，押小，还是按s进入设置？")
    while first not in foolfcmds:
        print("不要乱输命令！当心FBI红色警告！")
        first=input("押大，押中，押小，还是按s进入设置？")
    if first=='s':
        os.system("cls")
        a=input("下次骰子的点数设置成什么？")
        dicenumber=int(a)
        firstinto=False
        return 2
    second=input("押单还是押双？")
    while second not in seccmds:
        print("不要乱输命令！当心FBI红色警告！")
        second=input("押单还是押双？")
    for i in range(1,21):
        os.system("cls")
    ...
    show_game_info(dicenumber,level)
    print("获得数字：" + str(dicenumber))
    num=Dicenum(dicenumber)
    gamejudge = 0
    if num.bms == first:
        gamejudge += 1
...
def play():
    global money,info,firstinto,april_fool
    os.system("cls")
    rawlevel = info[1]
    level=int(rawlevel)
    if level >= 11:
        print("哇，你是赌神！您已经达成了Level达到11的成就，您将会获得十万元的奖励！接下来您的Level会被重置为1...")
        info[1]=1
        info[2]+=100000
        rawlevel=info[1]
        money=info[2]
    ...
    while cmd not in commands:
        '''
        if cmd=="7Mick-WindowsMetro" and time.localtime()[0:3]==(2022,4,1):
            april_fool=True
        '''
        print("不要乱输命令！当心FBI红色警告！")
        cmd = input("每次游戏需要花费$"+ str(gamecost) +"，继续吗？（按p继续，按e退出）")
    ...
    while first not in firstcmds:
        '''
        if first=="愚人节快乐！" and april_fool:
            ret=fool_play()
            while ret==0 or ret==2:
                ret=fool_play()
            firstinto=True
            return 4
        '''
