...
shellcmds = ("coin","addcoin","","exit","setcoin","setlevel","level")
...
def shell(gamecost):
    ...
    global money,info
    money += gamecost
    print("Gambling v0.0.2 - Debugging Shell")
...
def play():
    ...
    level = info[1]
    if level >= 11:
        print("哇，你是赌神！你已经达成了Level达到11的成就，即将将您的Level重置为1...")
        info[1]=1
        os.system("cls")
    gamecost = 500-(level-1)*50
    show_game_info(0)
...
def main():
    ...
    while True:
        ...
        os.system("cls")
        print("天天爱博彩")
        print("欢迎，" + str(username) + "\t\t Lv." + str(level))
        print("")
...
if __name__ == "__main__":
    binpath = os.path.expanduser("~\\usrinfo.bin")

    if os.path.exists(binpath):
        with open(binpath,"rb") as file:
            try:
                info = pickle.load(file)
                if not isinstance(info,list) or not len(info) == 2:
